...
def letsmakerecipe():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            response = f"Error: {e}"
    response = make_response(render_template('lmr.html', username=current_user.username,response=response))
    response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0'
    response.headers['Pragma'] = 'no-cache'
    response.headers['Expires'] = '0'
    return response
...
@login_required
def chatwithus():
    response = make_response(render_template('chat.html', username=current_user.username))
    response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0'
    response.headers['Pragma'] = 'no-cache'
    response.headers['Expires'] = '0'
    return response
...
def custommeal():
    ...
    response = make_response(render_template('cmp.html', username=current_user.username,response=response))
    response.headers['Cache-Control'] = 'no-store, no-cache, must-revalidate, post-check=0, pre-check=0'
    response.headers['Pragma'] = 'no-cache'
    response.headers['Expires'] = '0'
    return response
